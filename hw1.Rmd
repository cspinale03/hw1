...
cont <- read_csv("contraceptive_use.csv")
```
...
```{r}
# Your solution here...
dat <- cont %>% filter(is_in_union == "Y") %>% mutate(Year = round((start_date+end_date)/2), cp = contraceptive_use_modern * 100)
names(dat)[names(dat) == "division_numeric_code"] <- "iso"
dat <- dat %>% select(Year, iso, cp)
est2 <- merge(est, dat, by = c("iso"), all = TRUE) 
...
https://github.com/cspinale03/hw1
